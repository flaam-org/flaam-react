...
function ThemeChangeFAB() {
  ...
  return (
    <Popover as="div" className="w-10 h-10 fixed bottom-10 left-10 rounded-full shadow-xl text-black bg-gray-50 flex items-center justify-center">
      {({ open, close }) => (
        <>
          <Popover.Button className="w-3/4 h-3/4" >
            <ChevronDownIcon className={`w-full h-full ${open ? 'rotate-180' : 'rotate-0'} transition-transform ease-in-out duration-150`} />
          </Popover.Button>
          <Transition
            as={Fragment}
          ...
          >
            <Popover.Panel className="absolute bottom-12 p-3 rounded-md bg-gray-50 ">
