...
const TTab = ({ children }) => {
  return (
    <Tab
      ...
      className={({ selected }) =>
        joinClassNames(
          'w-full py-2 capitalize leading-5 focus:outline-none select-none',
          selected
            ? 'bg-transparent border-b-2 border-blue-500 text-green-600 font-bold '
            : 'text-green-400/50 hover:text-green-400/90 border-b-2 border-transparent hover:border-blue-500/50 '
        )
      }>{children}</Tab>
  )
}
